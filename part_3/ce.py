...
class CEDriver(NetworkDriver):
    ...
    def get_facts_ar(self):
        ...
        show_serial_number = self.device.send_command('display sn')

        # os_version/uptime
        for line in show_ver.splitlines():
            if 'VRP (R) software' in line:
                # VRP (R) software, Version 5.160 (AR1200 V200R007C00SPCb00)            
                search_result = re.search(r"(?P<os_version>Version .*)", line)
                os_version = search_result.group('os_version')

            if 'uptime is' in line:
                    
                #uptime = self._parse_uptime(line)
                search_result = re.search(r"(?P<uptime>( \d+ weeks,)*" \
                                           "(\d+ days,)* (\d+ hours,)* (\d+ minutes)*)", line)
                uptime = search_result.group('uptime')
                break
                
        # serial_number/model        
        for line in show_serial_number.splitlines():
            if 'AR' in line:
                # 0     --  AR1220F          2102350RUSDMGB001375     02350RUS 
        ...
        return {
            'uptime': uptime, # MPU 0(Master) : uptime is 2 weeks, 5 days, 9 hours, 7 minutes
            'vendor': vendor,
            'os_version': py23_compat.text_type(os_version), # VRP (R) software, Version 5.160 (AR1200 V200R007C00SPCb00)
            'serial_number': py23_compat.text_type(serial_number),
            'model': py23_compat.text_type(model),
            'hostname': py23_compat.text_type(hostname),
            'fqdn': fqdn,  # ? fqdn(fully quahulified domain name)
